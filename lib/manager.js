...
const {get_perm, is_static_proxy, is_mobile,
    get_password,get_uname} = require('../util/zones.js');
...
function Manager(argv){
    ...
    try {
        ...
        zerr.notice('Manager started %s', pkg.version);
        ...
        this.log.error('Manager constructor: %s', zerr.e2s(e));
        throw e;
    }
}
...
E.prototype.get_first_actions_data = function(){
    ...
    if (file.exists(filename))
    {
        ...
        } catch(e){ this.log.error('get first actions: %s', zerr.e2s(e)); }
    }
    return {sent: {}, sending: {}, pending: []};
};
...
function*mgr_stop(_this, reason, force, restart){
    ...
    _this.log.notice('Manager stopped:\n%s', reason);
...
function*mgr_create_single_proxy(_this, proxy_config, conf){
    ...
    if(conf.zone.startsWith("http://")){
       //allproxy format
        var uri = new URL(conf.zone);
        conf.proxy =  uri.hostname;
        conf.proxy_port = uri.port;

        if(conf.password.length > 1){
            var authArr = conf.password.split(":");
            conf.customer = authArr[0];
            conf.password = authArr[1];
        }
        //conf.zone = "@";
    }
...
E.prototype.proxy_create = etask._fn(
    ...
    this.on('uncaught', e=>{
        _this.log.error('proxy create: '+zerr.e2s(e));
    });
...
E.prototype.proxy_delete = etask._fn(function*mgr_proxy_delete(_this, port){
    this.on('uncaught', e=>{
        _this.log.error('proxy delete: '+zerr.e2s(e));
    });
    const proxy = _this.proxy_ports[port];
    if (!proxy)
        return;
    proxy.stop();
    proxy.on('stopped', this.continue_fn());
    yield this.wait();
    proxy.destroy();
...
function*mgr_proxy_create_api(_this, req, res){
    ...
    _this.log.info('proxy_create_api');
    let port;
...
function*mgr_proxy_update_api(_this, req, res){
    _this.log.info('proxy_update_api');
...
E.prototype.proxy_banips_api = function(req, res){
    ...
    const success = ips.every(ip=>proxy.banip(ip, ms, null, domain));
    if (success)
...
E.prototype.proxy_banip_api = function(req, res){
    ...
    if (!ip || !util_lib.is_ip(ip))
        throw `No ip provided`;
    if (proxy.banip(ip, ms, null, domain))
        return res.status(204).end();
...
E.prototype.get_banlist_api = function(req, res){
    ...
    if (req.query.full)
    {
        return res.json({ips: [...server.banlist.cache.values()].map(
...
function*mgr_proxy_delete_api(_this, req, res){
    _this.log.info('proxy_delete_api');
    const port = +req.params.port;
...
function*mgr_proxies_delete_api(_this, req, res){
    _this.log.info('proxies_delete_api');
...
E.prototype.test_port = etask._fn(function*lum_test(_this, proxy, headers){
    ...
    try {
        const r = yield util_lib.json({
            ...
            method: 'GET',
            proxy: `http://127.0.0.1:${proxy.port || proxy.opt.port}`,
...
function*mgr_refresh_ip(_this, ip, port){
    ...
    if (new_ips.error)
        return _this.log.warn('Refreshing IP failed: %s', new_ips.error);
...
E.prototype.add_www_whitelist_ip_api = function(req, res){
    ...
    this.set_www_whitelist_ips(this._defaults.www_whitelist_ips.concat(ip));
    this.config.save();
    res.send('OK');
};
...
E.prototype.logged_update = etask._fn(function*mgr_logged_update(_this, ua){
    console.log("logged_update begin")
    ...
    console.log("logged_update: " + _this.logged_in)
...
E.prototype.addBasicAuth = etask._fn(
    function*setBasicAuth(_this,  originaUrl,username, password){
        var urlParst = originaUrl.split("://");
        return `${originaUrl[0]}://${username}:${password}@${originaUrl[1]}`
    }
);
...
function*mgr_login_user(_this, token, username, password, customer){
    ...
    try {
        if (login_failed)
            config = yield _this.get_lum_local_conf(customer, token);
        console.log("get_lum_local_conf success")
    } catch(e){
        if (!e.status)
            throw e;
...
E.prototype.get_ip = etask._fn(function*mgr_set_location(_this){
    ...
    _this.opts.current_ip = res.body.clientIp;
});
...
E.prototype.run_cluster = function(){
    ...
    zerr.notice('Master cluster setting up '+num_workers+' workers');
...
function*mgr_run_cpu_usage_monitoring(_this){
    while (_this.is_running)
    {
        ...
        if (level)
            _this.log[level]('High CPU usage: '+usage);
